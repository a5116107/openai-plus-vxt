...
| 全量测试 | `pnpm exec tsx --test <tests 下全部 *.test.ts 与 *.test.mjs>` | PASS，168/168 |
| Live readiness 单元 | `pnpm test:live-readiness` | PASS，15/15；显式 token 与 session 文件都必须通过 `/backend-api/me` 服务端 2xx，三阶段必须互异，组件化 SOCKS5 构造需完整国家/会话映射，多方式计划、Saved Payment 总门、连续稳定窗口和历史脱敏均有反例覆盖 |
| Xray live runner | `node --test tests/live-readiness-xray-runner.test.mjs`；`OPX_XRAY_CONFIG_WAIT_SECONDS=1 pnpm audit:live:xray` | PASS，3/3；无配置 smoke 按预期退出 1 并返回 `XRAY_CONFIG_WAIT_TIMEOUT`，子进程已退出且三个端口已释放 |
| Live readiness 当前审计 | `node scripts/live-readiness-audit.mjs`，2026-08-10 11:25 UTC 第 16 次观测 | PASS（审计完成）；72 节点中 55 个成功、27 个不同出口；latest 总出口数为 4，Auth=JP、Checkout=SG、Billing=US，`targetStabilityReady`、`exitDiversityReady`、`multiStageEgressReady`、`probePlanReady` 均为 true，新服务端身份门与支付为 false，`fullLiveReady=false` |
...
| 身份服务端验证 | `validateIdentityToken()` 单元反例；固定 `/backend-api/me` 只读请求 | PASS；仅本地未过期 JWT 不再满足 `identityReady`，验证过程不读取响应体，非 2xx 与网络错误均 fail-closed |
...
| QR-16 全仓结构基线 | 显式 `git ls-files` 清单 + `structural-quality --strict` | PASS（基线判定），170 文件、91 finding（84 advisory、7 baseline）、0 blocker；`runner-format.ts` 告警为 0 |
| QR-17 可执行结构门 | `pnpm test:structural-baseline` | PASS，1/1；Git 派生范围 170 文件，91 finding（84 advisory、7 baseline）、0 blocker，`runner-format.ts` 告警为 0 |
...
结构预算说明：本轮在既有高风险入口拆分、fallback 收敛和 DOM 挂载治理基础上，新增可复跑的 170 文件全仓基线、收敛 `runner-format.ts` 并将基线接入自动测试；其余 91 个 finding 按 84 advisory、7 baseline 登记，不写成零债务或严格全仓无告警。
