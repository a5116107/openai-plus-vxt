...
| 全量测试 | `pnpm tsx --test tests/*.test.ts` | PASS，145/145 |
...
| QR-13/QR-15/QR-16 SSOT 一致性 | `pnpm test:ssot` | PASS，6/6；领域三件套、非完成任务根汇总、未勾选外部门、live 输入完整性、当前证据时效与结构基线均通过 |
...
结构预算说明：本轮在既有高风险入口拆分、fallback 收敛和 DOM 挂载治理基础上，新增可复跑的 168 文件全仓基线并收敛 `runner-format.ts`；其余 91 个 finding 按 84 advisory、7 baseline 登记，不写成零债务或严格全仓无告警。
