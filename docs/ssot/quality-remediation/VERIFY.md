...
| Firefox lint | `pnpm dlx web-ext lint --source-dir .output/firefox-mv2 --output json` | PASS，0 error / 0 notice / 28 warnings；26 条为设置页动态模板的 `UNSAFE_VAR_ASSIGNMENT`，2 条为 Firefox manifest 最低版本兼容提示，均不构成发布阻断 |
| QR-10 focused regression | `pnpm tsx --test tests/qr10-structure.test.ts` | PASS，3/3；脱敏日志、国家出口解析、支付方式报告模块均通过 |
| QR-10 shape delta | `git diff --stat` + `pnpm compile` | PASS；background -186 行、probe service -87 行、probe state -84 行、settings -44 行；新增职责模块无循环依赖 |
...
结构预算说明：本轮完成四个高风险入口的职责拆分和静默 fallback 收敛；跨 184 文件迁移的 change/shape budget 继续为 advisory，不写成严格预算 PASS。
