...
- 2026-08-10 外部门复测（06:54 UTC，第 10 条脱敏历史）：直连与 `127.0.0.1:10808` 经 sing-box 访问 Cloudflare/ChatGPT trace 均为 HTTP 200、TW/TPE；按 ChatGPT 成功观测去重的唯一出口 IP 为 1 个，仍不满足 3 个互异出口；QR-20 最近 3 次窗口连续成功 3 次、稳定门为 true，身份、出口多样性、三阶段出口、Saved Payment 与多方式计划门均为 false，`fullLiveReady=false`。10 条历史及 latest 经敏感形状扫描均无 IP、凭据或卡数据。`7890` 与 `18090` 当前均未监听。2026-08-09 隔离 profile 副本直连及经 SG 出口的浏览器 Session 均为 HTTP 403，当前默认 live session 目录仍为 0 个文件/0 个有效会话；Saved Payment preflight 的 profile、测试支付输入和后端配置均未注入，未形成新的账号、access token、支付方式或资格命中证据。
...
| 出口互异性 | 直连与 `10808` 匿名出口等同性校验一致；`7890` 与 `18090` 均未监听 | 阻塞多出口 |
