...
- 2026-08-10 外部门复测：直连与 `127.0.0.1:10808` 经 sing-box 访问 Cloudflare/ChatGPT trace 均为 HTTP 200、SG，匿名出口等同性校验一致，因此仍只有一个实际出口；QR-20 顺序审计首轮仅 1 次成功、稳定门为 false，第二轮及后续复测达到最近 3 次窗口内连续成功 3 次、稳定门为 true。`7890` 与 `18090` 当前均未监听。2026-08-09 隔离 profile 副本直连及经 SG 出口的浏览器 Session 均为 HTTP 403，当前默认 live session 目录仍为 0 个文件/0 个有效会话；Saved Payment preflight 的 profile、测试支付输入和后端配置均未注入，未形成新的账号、access token、支付方式或资格命中证据。
