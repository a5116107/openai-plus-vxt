# -*- coding: utf-8 -*-
"""AT -> ChatGPT page-context probe (real Mullvad browser + front proxy).
Verifies whether a Bearer AT can be used inside a real chatgpt.com page context
(which passes Cloudflare) to reach /backend-api/me, create hosted checkout,
and whether the oaics_ short link can be resolved without a browser login session.
Token is never printed or persisted; outputs are redacted."""
...
EVIDENCE = ROOT / ".context-snapshots" / f"live-at-page-probe-{VERSION}"
...
const summarize = async (label, url, opts = {}) => {
  ...
  try {
    const response = await fetch(url, {credentials: 'include', cache: 'no-store', ...(opts.fetch || {})});
    ...
    if (body && typeof body === 'object' && !Array.isArray(body)) {
      ...
      if (body.user && typeof body.user === 'object') {
        ...
        rec.userName = mask(body.user.name || '');
        rec.hasUser = true;
      }
      ...
      if (body.accounts && Array.isArray(body.accounts)) rec.accountCount = body.accounts.length;
      if (body.plan_type) rec.planType = mask(body.plan_type);
      if (body.email) rec.email = mask(body.email);
    }
    rec.bodyPreview = mask(text).slice(0, 500);
...
(async () => {
  const out = {origin: location.origin, path: location.pathname, ua: navigator.userAgent.slice(0, 80)};
  out.baselineSession = await summarize('session-noauth', '/api/auth/session', {fetch: {headers: {Accept: 'application/json'}}});
  out.meNoAuth = await summarize('me-noauth', '/backend-api/me', {fetch: {headers: {Accept: 'application/json'}}});
  out.meBearer = await summarize('me-bearer', '/backend-api/me', {fetch: {headers: {Accept: 'application/json', Authorization: 'Bearer ' + token}}});
  out.meHeader = await summarize('me-x-openai-access-token', '/backend-api/me', {fetch: {headers: {Accept: 'application/json', 'x-openai-access-token': token}}});
  out.sessionBearer = await summarize('session-bearer', '/api/auth/session', {fetch: {headers: {Accept: 'application/json', Authorization: 'Bearer ' + token}}});
  ...
  out.checkoutCreate = await summarize('checkout-create-hosted-ph', '/backend-api/payments/checkout', {
    fetch: {
      method: 'POST',
  ...
  out.oaicsId = oaics || '';
  if (oaics) {
    out.checkoutGet = await summarize('checkout-get-oaics-bearer', '/backend-api/payments/checkout/' + oaics, {fetch: {headers: {Accept: 'application/json', Authorization: 'Bearer ' + token}}});
    out.checkoutV1 = await summarize('checkout-v1-bearer', '/backend-api/checkout/v1/checkout/' + oaics, {fetch: {headers: {Accept: 'application/json', Authorization: 'Bearer ' + token}}});
    out.checkoutGetNoAuth = await summarize('checkout-get-oaics-noauth', '/backend-api/payments/checkout/' + oaics, {fetch: {headers: {Accept: 'application/json'}}});
  }
  done(out);
...
temp_root = None
try:
    temp_root = Path(tempfile.mkdtemp(prefix="opx-at-page-probe-"))
    ...
    result["addonRuntime"] = live.inspect_addon_runtime(driver, install["addonId"])
    result["lastStage"] = "addon-installed"

    # Real chatgpt.com page context: same-origin fetch bypasses the extension-context CF wall.
    driver.get("https://chatgpt.com/")
    time.sleep(5)
    result["pageProbe"] = driver.execute_async_script(PAGE_PROBE_JS, TOKEN)
    result["lastStage"] = "page-probe-complete"

    oaics = (result.get("pageProbe") or {}).get("oaicsId") or ""
    if oaics:
    ...
    result["ok"] = bool(result.get("pageProbe") and not result["errors"])
    ...
    if driver:
        try:
            if not result.get("ok"):
                result["browserEvidence"] = live.capture_browser_evidence(driver, result.get("browserEvidence") or {})
        except Exception:
            pass
        driver.quit()
    if temp_root:
